...
public class JobApplicationSession extends Session {
  ...
  private void handleConfirm(ConfirmEvent conf) {
    System.out.println("Aplicacao recebeu confirmacao de transformation: "
        + conf.getId());
  ...
  private void handleError(ErrorEvent alarm) {
    System.out.println("Aplicacao recebeu ERROR]");
   ...
   private class PrintReader extends Thread {
    ...
    public void run() {
      ...
      while (running) {
        ++rid;
        System.out.println();
        System.out.print("Aplicacao Iniciada: id " + rid + "> ");
        try {
